...
class EquipController extends BaseController {
  ...
  async add() {
    const { ctx } = this;
    const name = ctx.request.name;
    const { equip_name, brand, type, count, remark } = ctx.request.body;
    let data = {};
    await ctx.model.Equip.findOne({
      where: { equip_name, brand, type }
    })
      .then(async res => {
        if (!res) {
          await ctx.model.Equip.create({
            ...
            .then(async cres => {
              let id = cres.dataValues.id;
              let number = id + "";
              number = name + number.padStart(6, "0");

              await ctx.model.Equip.update({ number }, { where: { id, name } })
                .then(res => {
                  this.success(data);
                })
                .catch(err => {
